...
object Huffman {
  ...
  def insert(x:Char,xs: List[(Char,Int)]):List[(Char,Int)]= xs match{
    case List() => (x,1)::xs
    case (y::ys) => if(x == y._1)(x,1+y._2)::ys else y::insert(x,ys)
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match{
    case y1::y2::ys=> (makeCodeTree(y1, y2) :: ys).sortWith{(l,r)=> weight(l)<weight(r)}
    case y::Nil => y::Nil
    case Nil => Nil
  }
   ...
   */
    def until(s:List[CodeTree]=>Boolean , c: List[CodeTree]=>List[CodeTree])(tr: List[CodeTree]): List[CodeTree] = if(s(tr))tr else until(s,c)(c(tr))
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def tempDecode(tempTree : CodeTree, b:List[Bit]):List[Char] = (tempTree, b) match {
        case (Fork(l,r,c,w), 0::remB) => tempDecode(l, remB)
        case (Fork(l,r,c,w), 1::remB) => tempDecode(r, remB)
        case (Fork(l,r,c,w), Nil) => c
        case (Leaf(c,w), Nil) => c::Nil
        case (Leaf(c,w),remB) => c::tempDecode(tree, remB)
      }
      tempDecode(tree, bits)
    }
  /**
   ...
   */
    def convert(tree: CodeTree): CodeTable = {
      val cdTable = List()
      def temp(tr:CodeTree, acc:List[Bit]):CodeTable= tr match {
        case Fork(l,r,c,w) => temp(l, 0::acc):::temp(r,1::acc)
        case Leaf(c,w) => (c,acc.reverse)::cdTable
      }
      temp(tree, List())
    }
